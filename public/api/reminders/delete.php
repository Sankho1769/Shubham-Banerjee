...
try {
    // Trash by default; permanent delete only from the Trash view via ?permanent=1.
    if (!empty($data['permanent'])) {
        (new ReminderService())->forceDelete($id, $userId);
    } else {
        (new ReminderService())->trash($id, $userId);
    }
    Response::success([]);
    ...
    \App\Helpers\Logger::error('Reminder delete failed', ['error' => $e->getMessage()]);
    Response::error('Could not delete the reminder.', 500);
}
